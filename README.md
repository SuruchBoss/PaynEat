...
- **ทดสอบอัตโนมัติ 704 เคส** ก่อนปล่อยทุกครั้ง — ตั้งแต่กฎคิดเงินไปจนถึง flow เต็มร้าน 17 ขั้น
...
```bash
cd backend && npm test      # 298 เคส — รวมเทสต์ที่ไล่เส้นทางทั้งร้าน 17 ขั้น
cd app && flutter test      # 376 เคส — domain / controller / widget
cd app && flutter test test_e2e   # 30 เคส — แอปจริงคุยกับ backend จริง (ต้อง npm ci ใน backend ก่อน)
```
...
```bash
cd backend && npm test      # 298 เคส
cd app && flutter test      # 376 เคส
cd app && flutter test test_e2e   # 30 เคส (ต้อง npm ci ใน backend ก่อน)
```

**E2E — แอปจริงคุยกับ backend จริง (30 เคส)** — `app/test_e2e/` เปิด backend ตัวจริง
...
**Backend (298 เคส)** — `node:test` + `supertest` ยิงผ่าน HTTP จริงบนฐานข้อมูลแยกต่างหาก
...
**Flutter (376 เคส)** — แบ่งเป็น 3 ระดับ:
...
- [x] **เทสต์ integration ฝั่ง Flutter ยิงกับ backend จริง** — ทำแล้วเป็นชุด E2E `app/test_e2e/` (30 เคส)
