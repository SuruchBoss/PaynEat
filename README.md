...
9. **จะเด้งไปหน้าใบเสร็จ** → เห็นปุ่ม **"ขอใบกำกับภาษี"** ให้เลือกออกอย่างย่อ (ทันที) หรือเต็มรูป
   (กรอกชื่อ+ที่อยู่ลูกค้า) → ได้เอกสารพร้อมเลขที่รันต่อเนื่อง (เช่น `INV69-000001`) ทันที
10. **กลับไปดูผังโต๊ะ** → โต๊ะ A1 กลับมาเป็นสีเขียวเองแล้ว
11. **ออกจากระบบแล้วเข้าใหม่เป็น `admin`** → เปิดหน้า **ภาพรวม** จะเห็นยอดที่เพิ่งขายไปเข้ารายงานทันที
...
- ลองออกใบกำกับภาษีให้บิลเดียวกันซ้ำอีกใบ → ระบบปฏิเสธ (ออกได้ 1 ใบต่อบิลที่ยังไม่ถูกยกเลิก) — เข้าเป็น
  ...
  ด้วยเลขที่รันใหม่ (ดู `docs/DECISIONS.md` #16)
...
cd app && flutter test      # 229 เคส — domain / controller / widget
```
...
- **ออกใบกำกับภาษี** จากหน้าใบเสร็จของบิลที่จ่ายครบแล้ว เลือกออกอย่างย่อ (ทันที) หรือเต็มรูป
  ...
  ไม่ได้จนกว่าจะยกเลิกใบเดิมก่อน (สิทธิ์ยกเลิก manager ขึ้นไป ดู `docs/DECISIONS.md` #16)
...
- **ตั้งค่าร้าน** ชื่อร้าน, VAT, Service Charge, โหมดราคารวม VAT, เลขประจำตัวผู้เสียภาษี/ที่อยู่/
  สาขา (สำหรับออกใบกำกับภาษี — ไม่บังคับกรอกถ้าร้านไม่ได้จด VAT)
...
**Flutter (229 เคส)** — แบ่งเป็น 3 ระดับ:
...
| Controller | `receipt_controller_test.dart` | โหลดใบเสร็จจาก orderId, โหลดใบกำกับภาษีแบบเงียบๆ ถ้ายังไม่เคยออก (404 ไม่ใช่ error), สิทธิ์ยกเลิกใบกำกับภาษี (manager ขึ้นไป) |
...
- [x] **ใบกำกับภาษี** — ทำแล้ว: ตั้งค่าเลขผู้เสียภาษี/ที่อยู่/สาขาร้านได้ที่หน้าตั้งค่า ออกใบกำกับ
  ...
  (ดูหัวข้อ ✨ ฟีเจอร์ และ `docs/DECISIONS.md` #16) — **ยังไม่ทำ e-Tax invoice** (ยื่นอิเล็กทรอนิกส์
