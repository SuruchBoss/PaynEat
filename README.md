...
9. **กด "เก็บเงิน / ปิดบิล"** → เพราะผูกลูกค้าไว้ตั้งแต่ขั้นตอน 4 จะเห็นกล่อง **"แต้มสะสม"** โชว์
   แต้มคงเหลือของลูกค้า (ลูกค้าใหม่ยังไม่มีแต้มให้ใช้) ลองแยกจ่าย: จ่าย QR 100 บาทก่อน แล้วที่เหลือ
   จ่ายเงินสด (ระบบตัดยอดคงเหลือและคำนวณเงินทอนให้) — จ่ายครบแล้วลูกค้าจะได้แต้มสะสมอัตโนมัติ
   ตามยอดซื้อ (ค่าเริ่มต้น 25 บาท/แต้ม)
...
12. **ออกจากระบบแล้วเข้าใหม่เป็น `admin`** → เปิดหน้า **ภาพรวม** จะเห็นยอดที่เพิ่งขายไปเข้ารายงานทันที
    พร้อมกราฟรายชั่วโมงและสัดส่วนช่องทางชำระเงิน (เมนูขายดีดูได้ที่หน้า **รายงาน**)
13. **เปิดหน้า ลูกค้า/แต้มสะสม** (ไอคอนของขวัญ 🎁 ในเมนูด้านซ้าย — เห็นได้ทั้ง `admin`/`manager`)
...
14. **เปิดหน้า วัตถุดิบ/สต๊อก** (ไอคอนกล่อง 📦 ในเมนูด้านซ้าย) → เห็น "ปลาทับทิม" ถูกไฮไลต์แจ้งเตือน
    ของใกล้หมดให้ทันทีตั้งแต่ seed ข้อมูล
15. **เปิดหน้า ประวัติการทำรายการ** (ไอคอนนาฬิกา 🕘 ในเมนูด้านซ้าย — เห็นเฉพาะ `admin` เท่านั้น
...
- การกระทำที่เสี่ยงต่อการทุจริตหน้าร้านทั้งหมด (ยกเลิกออเดอร์, ยกเลิกรายการหลังส่งครัว, ให้ส่วนลด,
  ...
  ข้างบนนี้แล้วกลับไปดูหน้านั้น (ดูขั้นตอน 15 ในทัวร์ และ `docs/DECISIONS.md` #21)
...
```bash
cd backend && npm test      # 206 เคส — รวมเทสต์ที่ไล่เส้นทางทั้งร้าน 17 ขั้น
cd app && flutter test      # 278 เคส — domain / controller / widget
```
...
- **ตั้งค่าร้าน** ชื่อร้าน, VAT, Service Charge, โหมดราคารวม VAT, เลขประจำตัวผู้เสียภาษี/ที่อยู่/
  ...
  ได้ 1 แต้ม / มูลค่า 1 แต้มตอนใช้แลก)
...
```bash
cd backend && npm test      # 206 เคส
cd app && flutter test      # 278 เคส
```

**Backend (206 เคส)** — `node:test` + `supertest` ยิงผ่าน HTTP จริงบนฐานข้อมูลแยกต่างหาก
...
**Flutter (278 เคส)** — แบ่งเป็น 3 ระดับ:
...
| Domain | `demo_store_test.dart` | ตรวจว่าแยก `demo_store.dart` เป็น 15 ไฟล์แล้วเมธอดข้ามโดเมนยังทำงานถูก รวมถึง flow โปรโมชัน auto/โค้ด/ลบ/eligible list, flow ตัดสต๊อกอัตโนมัติ/ปิด-เปิดขายเมนูตามสต๊อกเต็มรูปแบบ, flow ออก/ยกเลิก/ออกใหม่ใบกำกับภาษีพร้อมเลขที่รัน, flow บันทึก audit log ครบทุก action เสี่ยง (ticket 08) และ flow ลูกค้า/แต้มสะสม: สร้าง/ค้นหาลูกค้า, ผูก customerId ตอนเปิดออเดอร์, สะสมแต้มครั้งเดียวตอนจ่ายครบ (รวมกรณีแยกจ่ายหลายรอบ), ใช้แต้มแลกส่วนลดโดยยอด amount ไม่เปลี่ยน และปฏิเสธการใช้แต้มที่ไม่ถูกต้องทุกกรณี (ticket 09) |
...
- [`docs/DECISIONS.md`](docs/DECISIONS.md) — บันทึกการตัดสินใจเชิงออกแบบ 22 ข้อ พร้อมข้อเสียที่ยอมรับ
