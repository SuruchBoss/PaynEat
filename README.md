...
- **ตั้งค่าร้าน** ชื่อร้าน, VAT, Service Charge, โหมดราคารวม VAT, เลขประจำตัวผู้เสียภาษี/ที่อยู่/
  ...
  (prefix + จำนวนหลัก PLU ให้ตรงกับตาชั่งของร้าน ดู `docs/DECISIONS.md` #49)
...
- **ลูกค้า/แต้มสะสม** เห็นได้ทั้ง `admin`/`manager` — ค้นหารายชื่อลูกค้าทั้งหมด กดเข้าไปดูประวัติ
  ...
  ตั้งวงเงิน/เครดิตเทอม/เลขผู้เสียภาษี/ที่อยู่ออกเอกสาร (ผู้จัดการขึ้นไป บันทึก audit log) เห็นยอดค้าง/วงเงิน
  คงเหลือ และปุ่มไปบัญชีลูกหนี้
...
- **สามภาษาทั้งระบบ: ไทย / English / 한국어** — สลับได้ที่หน้า **บัญชี** หรือหน้า **ตั้งค่า**
  ...
  ค่าที่ประทับไว้ตอนสั่ง คำแปลครบ 963 คีย์เท่ากันทุกภาษา ฟอนต์เกาหลีฝังมาในแอปแบบ subset เฉพาะ
  ตัวอักษรที่ใช้จริง (4 น้ำหนัก ~340KB) จึงไม่พึ่งฟอนต์ของเครื่อง มีเทสต์อ่านตาราง cmap ของไฟล์ฟอนต์
...
**E2E — แอปจริงคุยกับ backend จริง (42 เคส)** — `app/test_e2e/` เปิด backend ตัวจริง
...
**Backend (325 เคส)** — `node:test` + `supertest` ยิงผ่าน HTTP จริงบนฐานข้อมูลแยกต่างหาก
...
| Controller | `receivable_controllers_test.dart` | รวมยอดค้าง/เกินกำหนด, แยกบิลค้าง/บิลที่ยังไม่วางบิล/ใบวางบิลที่เปิดอยู่, สิทธิ์ยกเลิกเอกสาร (ผู้จัดการขึ้นไป), รับชำระสำเร็จส่งใบวางบิลที่เลือกแล้วโหลดยอดใหม่ / ไม่สำเร็จไม่โหลดซ้ำ (ticket 20) |
...
- [x] **รองรับภาษาเกาหลี** — ทำแล้ว: คำแปล 963 คีย์ครบทุกฟีเจอร์ (ตรวจแล้วว่าชุดคีย์ตรงกับ
...
- [x] **เทสต์ integration ฝั่ง Flutter ยิงกับ backend จริง** — ทำแล้วเป็นชุด E2E `app/test_e2e/` (42 เคส)
...
- **อ่านน้ำหนักสดจากตาชั่งผ่านสาย / สแกนด้วยกล้องมือถือ** — ตาชั่งแต่ละยี่ห้อพูดโปรโตคอลต่างกันและ Flutter
  web เข้าถึง serial port ไม่ได้ทุกเบราว์เซอร์ ใช้ฉลากตาชั่ง + เครื่องสแกนแบบคีย์บอร์ดแทน (ดู `docs/DECISIONS.md` #49)
- **ดอกเบี้ย/ค่าปรับจ่ายช้า, ใบลดหนี้แยกเอกสาร, ส่งใบวางบิลเป็น PDF ทางอีเมล** — ลดหนี้ด้วยการคืนเงินบิล
  ขายเชื่อแทน ดู/พิมพ์เอกสารจากจอ (ดู `docs/tickets/20-b2b-credit.md`)
