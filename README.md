...
- **ทดสอบอัตโนมัติ 876 เคส** ก่อนปล่อยทุกครั้ง — ตั้งแต่กฎคิดเงินไปจนถึง flow เต็มร้าน 17 ขั้น
...
> ไม่เห็น ถ้าจะลองหลายเครื่องพร้อมกันให้รันทางเลือก A หรือ B แล้วทุกเครื่องเปิดที่อยู่เดียวกัน (หรือลองทุกบทบาทบน
...
```bash
cd backend && npm test      # 355 เคส — รวมเทสต์ที่ไล่เส้นทางทั้งร้าน 17 ขั้น
cd app && flutter test      # 473 เคส — domain / controller / widget
...
- **สามภาษาทั้งระบบ: ไทย / English / 한국어** — สลับได้ที่หน้า **บัญชี** หรือหน้า **ตั้งค่า** และปุ่มลูกโลกบนหน้า
  เข้าสู่ระบบ/หน้าเมนู QR ของลูกค้า (เปิดครั้งแรกใช้ภาษาของเครื่อง)
  ...
  ค่าที่ประทับไว้ตอนสั่ง คำแปลครบ 1,094 คีย์เท่ากันทุกภาษา ฟอนต์เกาหลีฝังมาในแอปแบบ subset เฉพาะ
...
```bash
cd backend && npm test      # 355 เคส
cd app && flutter test      # 473 เคส
...
**Backend (355 เคส)** — `node:test` + `supertest` ยิงผ่าน HTTP จริงบนฐานข้อมูลแยกต่างหาก
...
**Flutter (473 เคส)** — แบ่งเป็น 3 ระดับ:
...
- [x] **รองรับภาษาเกาหลี** — ทำแล้ว: คำแปล 1,094 คีย์ครบทุกฟีเจอร์ (ตรวจแล้วว่าชุดคีย์ตรงกับ
...
- [x] **ตรวจ UX ก่อน UAT แบบไม่มีคนสอน (ลูกค้ากดเอง)** — ทำแล้ว: ไล่ทุกแท็บของทุกบทบาท + หน้าที่ต้องกดเข้าไป × 3 ภาษา ×
  ...
  กดไม่ได้ และ tooltip ปุ่มไอคอนทุกตัว (ดู `docs/DECISIONS.md` #62) — ที่ยังค้าง: ข้อความจาก backend ยังเป็นภาษาไทย
  อย่างเดียว, ปฏิทิน/ข้อความมาตรฐานของ Material ยังเป็นอังกฤษ, ชื่อพนักงาน/สาขาในข้อมูลสาธิตยังเป็นภาษาไทย
