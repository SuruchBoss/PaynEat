...
<p align="center">
  ...
  <img alt="Tests" src="https://img.shields.io/badge/tests-403%20passing-2F9E44">
...
control and 403 automated tests.
...
- Try requesting a tax invoice again for the same bill → rejected (only 1 active invoice per bill) —
  ...
  issue a fresh one for that bill again with a brand-new running number (see `docs/DECISIONS.md` #16)
...
cd app && flutter test      # 229 cases — domain / controller / widget
```
...
- **Issue a tax invoice** from the receipt page of any fully-paid bill — choose abbreviated (issued
  ...
  one is voided first (voiding requires manager role or above — see `docs/DECISIONS.md` #16)
...
**Flutter (229 cases)** — split into 3 levels:
...
| Controller | `receipt_controller_test.dart` | Loading a receipt by orderId, silently loading the tax invoice when none has been issued yet (404 isn't an error), tax-invoice void permission (manager role or above) |
...
- [x] **Tax invoice** — done: store tax ID/address/branch can be set from Settings; abbreviated/full tax
  ...
  and reissuing it is supported (see the ✨ Features section and `docs/DECISIONS.md` #16) — **e-Tax
