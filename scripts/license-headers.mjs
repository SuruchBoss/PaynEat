...
const CONFIG = {
  ...
  exclude: [
    /(^|\/)node_modules\//,
    /(^|\/)migrations\//,
    // Generated Dart (build_runner, gen-l10n) and build output.
    /\.(g|freezed|gr|mocks)\.dart$/,
    /(^|\/)(build|\.dart_tool)\//,
    /^app\/lib\/(generated|l10n)\//,
    // Platform scaffolding from `flutter create`. app/web/ has one hand-written file,
    // flutter_bootstrap.js, which carries the header; the rest of it is scaffolding.
    /^app\/(android|ios|macos|linux|windows)\//,
