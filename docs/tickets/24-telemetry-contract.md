...
- **log แบบ JSON บรรทัดละ object** แทน `morgan` และ `console.*` ทั้งหมด ช่องตามสัญญา:
  ...
  - `time`, `message`, `event`
  - `logging.googleapis.com/labels` ที่มี `app=payneat-pos-api` และ `correlation_id` และใส่ `location_code` เมื่อเกี่ยวกับสาขา
...
- **`http.request.completed`** พร้อม `httpRequest` โดย `requestUrl` เป็น path **ไม่รวม query string** และแปลง status เป็น
  `severity` ตามสัญญา
...
## Acceptance Criteria
- [ ] ทุกบรรทัด log ของ backend เป็น JSON ตามสัญญา v1 ไม่มี `morgan` / `console.*` เหลือใน `backend/src`
